...
const {storeVault,readVault}=await tsImport('../packages/control/vault.ts',import.meta.url);
...
test('Windows DPAPI roundtrip, encrypted bytes, create-only storage and private ACLs',{skip:process.platform!=='win32',timeout:60000},async t=>{
 ...
 const file=path.join(dir,'private','credential.dpapi');await storeVault(file,credential);
 ...
 const other=path.join(dir,'private','invalid.dpapi');const {writeFile}=await import('node:fs/promises');await writeFile(other,'garbage');await assert.rejects(readVault(other),{code:'vault_refused'});
});
