...
for(const kind of ['codex','claude-code'])test(`SDK client calls copied ${kind} v2 package with writes disabled`,{timeout:15000},async t=>{
 ...
 const env={...process.env};for(const key of Object.keys(env))if(key.startsWith('ZENITH_'))delete env[key];Object.assign(env,provenance.env,{ZENITH_API_VERSION:'2',ZENITH_URL:`http://127.0.0.1:${server.address().port}`,ZENITH_ALLOW_LOOPBACK_HTTP:'1',ZENITH_WORKSPACE_ID:'w',ZENITH_TOKEN:token});
