export const PROTOCOL_VERSIONS = ["2025-11-25", "2025-06-18", "2024-11-05"];
...
export const MIN_LINK_PROTOCOL_VERSION = 1;
...
export const READ_TOOLS = new Set(readNames);
export const isReadTool = (name) => typeof name === "string" && readNames.includes(name);
...
export function association(value) {
    ...
    for (const key of ["workspaceId", "projectId", "environmentId"]) {
        const v = value[key];
...
export function endpoint(origin, allowLoopbackHttp = false) {
    let url;
    try {
        url = new URL(origin);
    }
    catch {
        return fail("invalid_endpoint", "Set ZENITH_URL to the trusted Zenith origin.");
    }
...
export function validateRequest(value) {
    ...
    if ("params" in value && !isObject(value.params))
        return fail("invalid_request", "Request params must be an object.");
...
function validateMethod(message) {
...
export function validateResponse(message, value) {
    ...
    if ("error" in value) {
        ...
        return errorResponse(message.id ?? null, value.error.code, "Zenith rejected this protocol request. Inspect the server diagnostics using its request ID.");
    }
    const result = value.result;
    if (!isObject(result))
        return fail("invalid_response", "The MCP result must be an object.");
    if (message.method === "initialize") {
        if (!PROTOCOL_VERSIONS.includes(result.protocolVersion))
    ...
    if (message.method === "tools/list") {
        ...
        result.tools = result.tools.filter((tool) => {
            if (!isObject(tool) || !shortString(tool.name, 100))
                return fail("invalid_catalog", "A tool is missing its name.");
...
export class ZenithClient {
    ...
    get protocolVersion() { return this.version; }
    async request(raw, signal) {
        ...
        try {
            ...
            return fail("invalid_request", "Send a JSON-serializable request.");
        }
        ...
        let status, responseBytes = 0, outcome = "failure";
        ...
        try {
            ...
            if (!/^za_[A-Za-z0-9_-]{43}$/.test(token))
                fail("invalid_credential", "Provide a Zenith agent credential; browser cookies and provider keys are not accepted.");
            const headers = {
            ...
            if (message.id === undefined) {
                void response.body?.cancel().catch(() => { });
                fail("invalid_response", "An accepted notification must return HTTP 202 without an RPC response.");
            }
            ...
            if (!reader)
                fail("invalid_response", "Zenith returned an empty response; check endpoint compatibility.");
            const chunks = [];
            try {
                for (;;) {
                    ...
                    if (responseBytes > MAX_RESPONSE_BYTES)
                        fail("response_too_large", "Response exceeds 256 KiB. Narrow the query or use pagination.");
            ...
            if (message.method === "initialize" && isObject(result.result))
                this.version = String(result.result.protocolVersion);
            outcome = "success";
            return result;
        }
        catch (error) {
            if (abort.aborted)
                fail("request_aborted", "Read cancelled or timed out. No write was requested; reconnect and inspect again.");
            ...
            try {
                this.options.onDiagnostic?.({ component: "zenith-client", requestId, method: message.method, outcome,
                    durationMs: Date.now() - started, responseBytes, ...(status === undefined ? {} : { status }) });
            }
            catch { /* Diagnostics never alter results or permissions. */ }
        }
    }
}
