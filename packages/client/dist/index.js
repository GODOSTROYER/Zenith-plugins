    ...
    async request(raw, signal) {
        ...
        try {
            ...
            if (response.headers.has("mcp-session-id")) {
                await response.body?.cancel();
                fail("unsupported_transport", "A stateless Zenith endpoint is required; a session-based server was returned.");
            }
            ...
            if (message.method === "initialize" && isObject(result.result)) {
                ...
                if (!PROTOCOL_VERSIONS.includes(v))
            ...
            if (message.method === "tools/list" && isObject(result.result) && Array.isArray(result.result.tools))
                result.result.tools = result.result.tools.filter((t) => isObject(t) && READ_TOOLS.has(String(t.name)));
            return result;
        }
        catch (error) {
