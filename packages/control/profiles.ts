...
const definition=z.object({origin:z.string(),scope,allowLoopbackHttp:z.boolean().default(false),allowWrites:z.boolean().default(false),
  credentialKind:z.enum(['opaque','oauth']).default('opaque'),credential:z.discriminatedUnion('kind',[
    z.object({kind:z.literal('file'),path:z.string()}).strict(),z.object({kind:z.literal('dpapi'),path:z.string()}).strict(),z.object({kind:z.literal('environment'),variable:z.string().regex(/^[A-Z][A-Z0-9_]{1,100}$/)}).strict()
  ])}).strict();
...
const conflicting=['ZENITH_URL','ZENITH_WORKSPACE_ID','ZENITH_PROJECT_ID','ZENITH_ENVIRONMENT_ID','ZENITH_CONFIG_FILE','ZENITH_TOKEN_FILE','ZENITH_TOKEN_VAULT','ZENITH_CREDENTIAL_KIND','ZENITH_ALLOW_WRITES','ZENITH_ALLOW_LOOPBACK_HTTP'];
...
export async function profileCommand(args:string[]):Promise<unknown>{
  ...
  const flags=new Set(['--file','--name','--url','--workspace','--project','--environment','--loopback','--writes','--credential-kind','--token-file','--token-env']);
  ...
  if(fields['--token-file']&&fields['--token-env'])throw new ClientError('ambiguous_configuration','Choose a token file OR an environment variable.');
  ...
  if(verb==='list'){const doc=await loadProfiles(file);return {active:doc.active,profiles:Object.entries(doc.profiles).map(([name,p])=>({name,origin:p.origin,scope:p.scope,allowWrites:p.allowWrites,credentialKind:p.credentialKind}))};}
  ...
  const p=definition.parse({origin:fields['--url'],scope:{version:1,workspaceId:fields['--workspace'],...(fields['--project']?{projectId:fields['--project']}:{}),...(fields['--environment']?{environmentId:fields['--environment']}:{})},allowLoopbackHttp:fields['--loopback']==='1',allowWrites:fields['--writes']==='1',credentialKind:fields['--credential-kind']??'opaque',credential:fields['--token-file']?{kind:'file',path:fields['--token-file']}:{kind:'environment',variable:fields['--token-env']??'ZENITH_TOKEN'}});
